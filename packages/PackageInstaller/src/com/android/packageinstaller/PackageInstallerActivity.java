...
public class PackageInstallerActivity extends AlertActivity {
    ...
    private boolean isInstallRequestFromUnknownSource(Intent intent) {
        final String AURORA_PACKAGE_NAME = "com.aurora.store";
        if (AURORA_PACKAGE_NAME.equals(mCallingPackage)) {
            try {
                PackageInfo packageInfo = getPackageManager().getPackageInfo(
                        mCallingPackage, PackageManager.PackageInfoFlags.of(
                ...
                if (Signature.areEffectiveMatch(new Signature(AURORA_STORE_SIGNATURE), new Signature(outputStream.toByteArray()))) {
                    return false;
                }
